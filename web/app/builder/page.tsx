    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 glass-panel p-6 rounded-3xl space-y-6">
          <form onSubmit={handleSaveRoom} className="space-y-4">
            ...
            <div className="space-y-3 pt-2">
              <div>
                <div className="flex justify-between text-xs font-semibold text-slate-300 mb-1">
                  ...
                  <span className="text-indigo-400 font-mono">{rows} Rows</span>
                </div>
                <input
                  type="range"
                  min="2"
              ...
              <div>
                <div className="flex justify-between text-xs font-semibold text-slate-300 mb-1">
                  ...
                  <span className="text-indigo-400 font-mono">{benchesPerRow} Benches</span>
                </div>
                <input
                  type="range"
                  min="1"
            ...
            <div className="p-4 rounded-2xl bg-indigo-950/40 border border-indigo-500/30 flex items-center justify-between text-xs">
              <div>
                <div className="text-slate-400 font-medium">Calculated Capacity</div>
            ...
            <button
              type="submit"
              className="w-full py-3 rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold text-xs tracking-wide uppercase transition shadow-lg shadow-indigo-600/30 flex items-center justify-center gap-2"
            >
        ...
        <div className="lg:col-span-7 glass-panel p-6 rounded-3xl space-y-4">
          ...
          {/* Blackboard Stage */}
          <div className="py-2.5 px-4 rounded-xl bg-slate-950 border border-slate-800 text-center text-xs font-semibold text-slate-400 font-mono">
            🖥️ Front Screen / Invigilator Podium
          </div>

          {/* Rendered Live Blueprint Grid */}
          <div className="space-y-3 max-h-[440px] overflow-y-auto no-visible-scrollbar p-3 rounded-2xl bg-slate-950/40 border border-slate-900">
            {Array.from({ length: rows }).map((_, r) => (
              <div key={r} className="space-y-1">
                <div className="text-[10px] text-slate-500 font-mono">Row {r + 1}</div>
                <div
                    ...
                    >
                      ...
                      >
                        {Array.from({ length: studentsPerBench }).map((_, p) => (
                          <div
                            key={p}
                            className="py-1 rounded bg-indigo-500/10 border border-indigo-500/20 text-[9px] font-mono text-indigo-300 font-bold"
                          >
                            {["F-1", "S-1", "T-1", "F-2"][p]}
                          </div>
                        ))}
